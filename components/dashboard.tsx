...
import { Brain, Users, Code, TrendingUp, LogOut, User, History } from "lucide-react"
...
export function Dashboard() {
  ...
  const [currentView, setCurrentView] = useState<
    "dashboard" | "aptitude-test" | "group-discussion" | "coding-challenge" | "history" | "profile"
  >("dashboard")
  ...
  const modules = [
    {
      ...
      color: "bg-blue-500",
      action: () => setCurrentView("aptitude-test"),
  ...
  if (currentView === "aptitude-test") {
    return <AptitudeTest onBack={() => setCurrentView("dashboard")} />
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-primary/5 to-accent/5">
      ...
      <header className="border-b bg-background/80 backdrop-blur-sm">
        <div className="container mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center space-x-4">
            <Button variant="outline" size="sm" onClick={() => setCurrentView("history")} className="flex items-center space-x-2">
            ...
            <div className="flex items-center space-x-2">
              ...
              <span className="font-medium">{userProfile?.username || userProfile?.displayName}</span>
            </div>
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-muted-foreground">Continue your placement preparation journey</p>
        </div>
        ...
        <div className="grid md:grid-cols-4 gap-6 mb-8">
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{userProfile?.stats.totalScore || 0}</div>
            </CardContent>
          </Card>
        ...
        <div className="grid md:grid-cols-3 gap-6 mt-8">
          {modules.map((module) => (
            <Card key={module.id} className="hover:shadow-lg transition-shadow cursor-pointer relative">
              {/*   {isNew && (
            ...
            {userProfile?.history && userProfile.history.length > 0 ? (
              <div className="divide-y">
                {userProfile.history.slice(0, 5).map((activity, idx) => (
                  <div key={activity.id || idx} className="py-4 flex flex-col md:flex-row md:items-center md:justify-between">
                    <div>
                      <span className="font-semibold">{activity.title}</span>
                      {activity.score !== undefined && (
                        <span className="ml-2 text-primary font-bold">{activity.score} pts</span>
                      )}
                    </div>
